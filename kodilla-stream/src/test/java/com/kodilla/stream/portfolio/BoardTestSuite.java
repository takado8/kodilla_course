...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        Board project = prepareTestData();
        //When
        var tasksDurations = project.getTaskLists().stream()
                .filter(taskList -> taskList.getName().equals("In progress"))
                .flatMap(taskList -> taskList.getTasks().stream())
                .map(task -> Duration.between(task.getCreated().atStartOfDay(),
                        LocalDate.now().atStartOfDay()).toDays())
                .collect(Collectors.toList());
        var avgTaskDuration = tasksDurations.stream()
                .reduce(0L, (sum, current) -> sum += current) / tasksDurations.size();

        assertEquals(10, avgTaskDuration);
    }
}